...
# ====== دالة الصوت المجانية باستخدام edge_tts ======
...
def chat():
 try:
  ...
  # ====== كشف طلب الصور والفيديو (مجاني للجميع) ======
  ...
  # ====== تحليل الصورة المرفوعة (للأدمن فقط) ======
  ...
  # تحليل الصور المرفوعة (للأدمن)
  img_data=d.get("image",None)
  if img_data and is_admin:
  ...
  # صوت مجاني للجميع (edge-tts)
  try:
   gender=session.get('voice_gender','male')
   audio=generate_speech(reply,gender)  # الآن مجاني للجميع
  except Exception as e:
   print(f"⚠️ فشل الصوت: {e}");audio=None

...
- إذا كنت تعمل على النموذج الأساسي (المجاني): ابدع بكل ما أوتيت من قوة! لا تظهر بمظهر البوت البسيط؛ كن سريع البديهة، عالي الحماس، استغل ملف (knowledge.md) بذكاء، وقدم إجابات ممتعة ومليانة أفكار وحيوية تخلي المستخدم يقول: "يا رجل إذا هذا المجاني كذا، أجل وشلون المدفوع؟!".
- إذا كنت تعمل على النموذج المتقدم (المدفوع): أطلق "الوحش"! هنا الصقها ببعضها؛ تعمّق لأقصى درجة في ملف (knowledge.md)، وفعّل أدوات البحث المتقدم في الويب لجلب أحدث وأدق البيانات والتحليلات. لا تكتفي بالإجابة، بل حلل، قارن، وابتكر رؤى استراتيجية عميقة لا تخطر على البال مع حماس يملأ المكان.
...
def chat():
    try:
        ...
        response = client.chat.completions.create(
            model=model,
            messages=messages,
            max_tokens=1000 if model == "gpt-5.2" else 800,
            temperature=0.8
        )

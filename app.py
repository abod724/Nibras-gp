...
def chat():
    ...
    # رد خاص إذا المستخدم سأل عن البرمجة
    if user_msg and any(k in user_msg for k in ["برمج", "مطور", "سواك", "المبرمج"]):
        return jsonify({"reply": "تم تطويري وبرمجتي على يد مبرمجين ومطورين عالميين، ومهمتي إني أساعدك بأفضل شكل ممكن 🤖"})
    ...
    # 🔥 هنا الـ system_prompt الجديد بالكامل 🔥
    system_prompt = f"""
نبراس يتكلم باللهجة السعودية العامية بأسلوب سوالف طبيعي، ودّي، خفيف، ويطوّل شوي إذا الموضوع يحتاج. هدفه يخلي المستخدم يحس إنه يسولف مع شخص طبيعي بدون مبالغة في اللطافة أو العاطفة. نبراس يعطي حلول واقتراحات، ويساعد المستخدم بشكل عملي، ويتفاعل معه بطريقة مريحة.

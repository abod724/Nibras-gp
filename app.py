...
limiter=Limiter(key_func=get_remote_address,default_limits=["500 per day","30 per hour"])
...
// ====== دالة تنسيق النص المعدلة (الحل النهائي) ======
function formatBotText(t){ 
    let s=String(t||'');
    // تقسيم النص إلى فقرات بناءً على الأسطر الفارغة (سطرين متتاليين)
    let paragraphs = s.split(/\n\s*\n/);
    // داخل كل فقرة، نستبدل جميع أنواع الأسطر الجديدة بمسافات

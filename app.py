...
window.deleteMyData=function(){
 ...
 fetch('/delete_my_data',{method:'POST',headers:{'Content-Type':'application/json'}})
 .then(response=>response.json())
 .then(data=>{
  if(data.status==='success'){alert('✅ تم حذف جميع بياناتك بنجاح.');window.location.href='/'}
  else{alert('❌ فشل الحذف: '+(data.message||'خطأ غير معروف'))}
 })
 .catch(err=>{alert('❌ حدث خطأ في الاتصال.');console.error(err)});
};
})();</script></body></html>"""
...
def login():
 if request.method=='POST':
  ...
  if e==ae:
   ...
   session.clear();session['user_email']=e;return redirect(url_for('index'))
...
@app.route('/history')
def history():
 uid=get_user_id();cs=get_user_conversations(uid);return jsonify({"conversations":[{"id":c["id"],"title":c["title"]} for c in cs]})
...
# ====== لوحة تحكم الأدمن (مضافة فقط) ======
@app.route('/admin')
def admin_dashboard():
    if not session.get('admin_email') == "abdullaha0569361@gmail.com":
        return "🚫 هذه الصفحة خاصة بالأدمن فقط.", 403
    conn=get_db()
    users_count=conn.execute("SELECT COUNT(DISTINCT user_id) FROM conversations").fetchone()[0]
    total_convs=conn.execute("SELECT COUNT(*) FROM conversations").fetchone()[0]
    today=datetime.now().strftime("%Y-%m-%d")
    today_convs=conn.execute("SELECT COUNT(*) FROM conversations WHERE timestamp LIKE ?", (today+'%',)).fetchone()[0]
    recent=conn.execute("SELECT user_id, title, timestamp FROM conversations ORDER BY timestamp DESC LIMIT 10").fetchall()
    conn.close()
    recent_html=""
    for row in recent:
    ...
    <head><meta charset="UTF-8"><meta name="viewport" content="width=device-width, initial-scale=1.0"><title>لوحة تحكم نبراس</title>
    <style>body{{font-family:'Segoe UI',Tahoma;background:#0d1117;color:#c9d1d9;padding:20px;margin:0}}.container{{max-width:600px;margin:auto}}h1{{color:#58a6ff;text-align:center}}.card{{background:#161b22;border-radius:15px;padding:15px;margin:15px 0;border:1px solid #30363d}}.stat{{display:flex;justify-content:space-between;padding:8px 0;border-bottom:1px solid #21262d}}.stat:last-child{{border:none}}.num{{color:#58a6ff;font-weight:bold;font-size:18px}}.conv-item{{padding:10px 0;border-bottom:1px solid #21262d}}.conv-item small{{color:#8b949e;display:block;font-size:12px}}.back{{display:block;text-align:center;color:#58a6ff;text-decoration:none;margin-top:20px}}
    </style></head>
    <body><div class="container"><h1>📊 لوحة تحكم نبراس</h1>
    <div class="card"><div class="stat"><span>👥 إجمالي المستخدمين</span><span class="num">{users_count}</span></div>
    <div class="stat"><span>💬 إجمالي المحادثات</span><span class="num">{total_convs}</span></div>
    <div class="stat"><span>📅 محادثات اليوم</span><span class="num">{today_convs}</span></div></div>
    <div class="card"><h3>🕒 آخر 10 محادثات</h3>{recent_html}</div>
    <a href="/" class="back">⬅ العودة للرئيسية</a></div></body></html>
    """
    return html
# ============================================
def get_user_id():
 ...
 try:
  ...
  if not is_admin:
   if not check_guest_limit_safe(uid):
    ...
    sm[uid].append({"role":"assistant","content":reply_limit})
   ...
   if cached:
    if cid is None:sm[uid]=[]
    sm[uid].append({"role":"user","content":um})
    sm[uid].append({"role":"assistant","content":cached})
    nid=save_user_conversation(uid,sm[uid],cid)
  ...
  is_media=is_image_request(um) or has_image
  if is_media and not is_admin:
   reply="عذراً، ميزة توليد الصور والفيديو وتحليل الصور والبحث المباشر متاحة لحساب الأدمن فقط حالياً للحفاظ على الموارد. تقدر تسألني أي سؤال نصي وأنا حاضر لك من ملف المعرفة أو من معرفتي العامة.\n\n💡 جرب أدواتنا المجانية: /tools"
   if cid is None:sm[uid]=[]
   sm[uid].append({"role":"user","content":um})
  ...
  if cid is None:sm[uid]=[]
  model=OPENAI_MODEL
  use_web=True if is_admin else False
  allow_img=True if is_admin else False
  ...
  sm[uid]=server_hist
  ch=server_hist[-10:];msgs=[{"role":"system","content":SP}]
  ...
  try:gender=session.get('voice_gender','male');audio=generate_speech(reply,gender) if is_admin else None
